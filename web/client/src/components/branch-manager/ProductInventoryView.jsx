  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Quản lý tồn kho & Vắc-xin</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-3xl font-bold text-orange-600 mt-2">{getLowStockProducts().length}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
          <p className="text-sm text-gray-600">Tổng giá trị tồn kho</p>
          <p className="text-2xl font-bold text-green-600 mt-2">
            {filteredProducts.reduce((sum, p) => sum + (p.SoLuongTon * p.DonGia || 0), 0).toLocaleString('vi-VN')} VNĐ
          </p>
        </div>
      </div>
              ...
              {popularVaccines.map((vaccine, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-gray-900">{vaccine.TenVacXin}</p>
                    </div>
                  </div>
      ...
      {getLowStockProducts().length > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
          <p className="text-orange-800 font-semibold">
            ⚠️ Cảnh báo: Có {getLowStockProducts().length} sản phẩm sắp hết hàng (dưới 10 đơn vị)
          </p>
        </div>
      )}
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredProducts.map((product) => (
                  <tr 
                    key={product.MaSP} 
                    className={`hover:bg-gray-50 ${product.SoLuongTon < 10 ? 'bg-orange-50' : ''}`}
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900">
                      ...
                      {product.LoaiSP === 'Vaccine' && product.LoaiVaccine && (
                        <div className="text-xs text-gray-500 mt-1">{product.LoaiVaccine}</div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        product.LoaiSP === 'Vaccine' ? 'bg-blue-100 text-blue-800' : 
                        product.LoaiSP === 'Thức ăn' ? 'bg-green-100 text-green-800' :
                        product.LoaiSP === 'Thuốc' ? 'bg-purple-100 text-purple-808' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {product.NgaySX ? new Date(product.NgaySX).toLocaleDateString('vi-VN') : 'N/A'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">
                      {product.DonGia?.toLocaleString('vi-VN')} VNĐ
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm text-right font-semibold ${
                      product.SoLuongTon < 10 ? 'text-orange-600' : 'text-gray-900'
                    }`}>
                      {product.SoLuongTon}
                      {product.SoLuongTon < 10 && ' ⚠️'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900">
                      {(product.SoLuongTon * product.DonGia).toLocaleString('vi-VN')} VNĐ
                    </td>
                  </tr>
                ))}
              </tbody>
