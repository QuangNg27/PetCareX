  ...
  return (req, res, next) => {
    try {
      if (!req.user) {
        return res
          .status(401)
          .json(RESPONSE_FORMAT.error(ERROR_MESSAGES.UNAUTHORIZED, 401));
      }
      ...
      res
        .status(500)
        .json(RESPONSE_FORMAT.error(ERROR_MESSAGES.SERVER_ERROR, 500));
    }
  };
};
...
const isSales = (req) => {
  return req.user?.role === USER_ROLES.SALES;
};
