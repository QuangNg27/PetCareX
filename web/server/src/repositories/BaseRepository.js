        ...
        try {
            ...
            for (const [key, data] of Object.entries(tvpParams)) {
                ...
                if (key === 'CT_SanPham') {
                    ...
                    table.columns.add('MaDV', sql.Int);
                    table.columns.add('MaTC', sql.Int);
                    table.columns.add('MaKB', sql.Int);
                    table.columns.add('MaTP', sql.Int);
                    table.columns.add('GiaApDung', sql.Decimal(10, 2));
                    
                    data.forEach(row => {
                        table.rows.add(row.MaCN, row.MaDV, row.MaTC, row.MaKB, row.MaTP, row.GiaApDung);
                    });
                }
