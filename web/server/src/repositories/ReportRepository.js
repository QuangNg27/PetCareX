...
class ReportRepository {
    ...
    async getInactiveCustomerCount(monthsInactive = 6, branchId = null) {
        try {
            const request = await poolRequest();
            request.input('MonthsInactive', monthsInactive);
            const cutoffDate = new Date();
            cutoffDate.setMonth(cutoffDate.getMonth() - monthsInactive);
            request.input('CutoffDate', cutoffDate.toISOString().split('T')[0]);
            let whereClause = '';
            if (branchId) {
                request.input('BranchId', branchId);
    ...
    async getRevenueByMonth(year, branchId = null) {
        try {
            ...
            request.input('NgayBD', NgayBD);
            request.input('NgayKT', NgayKT);
    ...
    async getRevenueByYear(startYear, endYear, branchId = null) {
        try {
            ...
            request.input('StartYear', startDate);
            request.input('EndYear', endDate);
