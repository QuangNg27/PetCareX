...
class ProductService {
  ...
  async getProduct(productId) {
    ...
    const inventory = await this.productRepository.getProductInventory(
      productId
    );
  ...
  async getProductsByBranch(branchId, filters = {}) {
    const products = await this.productRepository.getProductsByBranch(
      branchId,
      filters
    );
  ...
  async updateProductPrice(productId, priceData, userRole) {
    ...
    const result = await this.productRepository.updateProductPrice(
      productId,
      price,
      effectiveDate
    );
  ...
  async getProductPriceHistory(productId) {
    ...
    const history = await this.productRepository.getProductPriceHistory(
      productId
    );
  ...
  async getLowStockAlert(branchId = null, threshold = 10) {
    const lowStockProducts = await this.productRepository.getLowStockProducts(
      branchId,
      threshold
    );
  ...
  async getMedicines() {
    return await this.productRepository.getMedicines();
  }

  async getVaccines() {
    return await this.productRepository.getVaccines();
  }
