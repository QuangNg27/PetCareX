...
class AuthService {
  ...
  async login(credentials) {
    ...
    let branchId = null;
    if (account.MaNV) {
      const employeeInfo = await this.authRepo.getEmployeeBranchInfo(
        account.MaNV
      );
      if (employeeInfo) {
  ...
  async getProfile(username) {
    ...
    // Include branch info (MaCN) for employee accounts when available
    let branchId = null;
    if (account.MaNV) {
      const employeeInfo = await this.authRepo.getEmployeeBranchInfo(
        account.MaNV
      );
      if (employeeInfo) branchId = employeeInfo.MaCN;
    }
  ...
  async refreshToken(refreshToken) {
    try {
      ...
      // Get user info
      const account = await this.authRepo.findAccountByUsername(
        decoded.username
      );
      if (!account) {
        throw new AppError("Tài khoản không tồn tại", 404);
